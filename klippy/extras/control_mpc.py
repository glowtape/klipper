...
class ControlMPC:
    def __init__(self, heater, config, override_block_temp=AMBIENT_TEMP, config_override=None):
        ...
        self.const_filament_density = config.getfloat("filament_density", 1.20, minval = 0.5)
        self.const_filament_heat_capacity = config.getfloat("filament_heat_capacity", 1.80, minval = 0.5)
        self.const_maximum_retract = config.getfloat("maximum_retract", 2.0, minval = 0)

        filament_temp_src_raw = config.get("filament_temp_src", "ambient").lower().strip()
        if filament_temp_src_raw == FILAMENT_TEMP_SRC_SENSOR:
            self.filament_temp_src = (FILAMENT_TEMP_SRC_SENSOR,)
        elif filament_temp_src_raw == FILAMENT_TEMP_SRC_AMBIENT:
        ...
        if fan_name is not None:
            fan_obj = self.printer.load_object(self.config, fan_name, None)
            if fan_obj is None:
    ...
    def cmd_MPC_SET(self, gcmd):
        ...
        if gcmd.get("FAN_AMBIENT_TRANSFER", None):
            try:
                ...
                raise gcmd.error(
                    "Error on '%s': unable to parse FAN_AMBIENT_TRANSFER\n"
        ...
        if temp is not None:
            ...
            if temp == "sensor":
                ...
                try:
                    ...
                    raise gcmd.error(
                        "Error on '%s': unable to parse FILAMENT_TEMP\n"
    ...
    def cmd_MPC_CALIBRATE(self, gcmd):
        cal = MpcCalibrate(self.config, self.printer, self.heater, self)
        cal.run(gcmd)
    ...
    def check_valid(self):
        ...
        raise self.printer.command_error(
            "Cannot activate '%s' as MPC control is not fully configured.\n\n"
            "Run 'MPC_CALIBRATE' or ensure 'block_heat_capacity', 'sensor_responsiveness', and "
            "'ambient_transfer' settings are defined for '%s'."
            % (name, name)
        )
    ...
    def temperature_update(self, read_time, temp, target_temp):
        ...
        if self.cooling_fan and len(self.const_fan_ambient_transfer) > 1:
            ...
            below = self.const_fan_ambient_transfer[int(math.floor(fan_break))]
            above = self.const_fan_ambient_transfer[int(math.ceil(fan_break))]
            if below != above:
                frac = fan_break % 1.0
...
class MpcCalibrate:
    def __init__(self, config, printer, heater, orig_control):
        self.config = config
    ...
    def run(self, gcmd):
        ...
        try:
            ...
            fan_ambient_transfer = ", ".join(
                ["%.6g" % p for p in second_res["fan_ambient_transfer"]]
            )

            cfgname = self.heater.get_name()
            gcmd.respond_info(
                "Finished MPC calibration of heater '%s'\n"
                "Measured:\n "
                "  block_heat_capacity=%#.6g [J/K]\n"
                "  sensor_responsiveness=%#.6g [K/s/K]\n"
                "  ambient_transfer=%#.6g [W/K]\n"
                "  fan_ambient_transfer=%s [W/K]\n"
                % (cfgname, block_heat_capacity, sensor_responsiveness, ambient_transfer, fan_ambient_transfer)
            )

            configfile = self.heater.printer.lookup_object("configfile")
            configfile.set(cfgname, "control", "mpc")
            configfile.set(
                cfgname, "block_heat_capacity", "%#.6g" % block_heat_capacity
            )
            configfile.set(
                cfgname,
                "sensor_responsiveness",
                "%#.6g" % sensor_responsiveness,
            )
            configfile.set(
                cfgname, "ambient_transfer", "%#.6g" % ambient_transfer
            )
            configfile.set(
                cfgname,
        ...
        if self.orig_control.ambient_sensor is not None:
            ...
            while not self.printer.is_shutdown():
                ...
                if ret and not reported[0]:
                    gcmd.respond_info(
                        "Waiting for heater to drop below %g degrees celcius" % target
                    )
                    reported[0] = True
                if not ret:
                    break
    ...
    def transfer_test(
        ...
        if fan is None:
            ...
            gcmd.respond_info("Average stable power: %g W" % power_base)
        else:
            for idx in range(0, fan_breakpoints):
                speed = 1.0 * idx / (fan_breakpoints - 1)
                curtime = self.reactor.monotonic()
                fan.set_speed(speed)
                gcmd.respond_info("Waiting for temperature to stabilize")
                self.wait_stable(3)
                gcmd.respond_info(
                    "Temperature stable, measuring power usage with %.0f%% fan speed" % (speed * 100.0)
                )
                power = self.measure_power(
                    ambient_max_measure_time, ambient_measure_sample_time
                )
                gcmd.respond_info(
                    "%.0f%% fan average power: %.2f W" % (speed * 100.0, power)
                )
    ...
    def measure_power(self, max_time, sample_time):
        ...
        while not self.printer.is_shutdown():
            dt = eventtime - (
                last_time if last_time is not None else eventtime
            )
            if last_time is not None:
                status = self.heater.get_status(eventtime)
                samples.append((dt, status["power"] * dt * self.orig_control.const_heater_power))
                time += dt
            if time >= max_time:
                break
            last_time = eventtime
